...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            // Validation checks
            const validationResult = await this.validateRaid(attacker.id, target);
            if (!validationResult.valid) {
                return interaction.reply({
                    embeds: [this.createErrorEmbed(validationResult.reason)],
                    ephemeral: true
                });
            }
            ...
            const [attackerData, targetData] = await Promise.all([
                this.getParticipantData(attacker.id),
                this.getParticipantData(target.id)
            ]);
            
            // Start the automated raid battle
            const battleResult = await this.executeAutomatedBattle(attackerData, targetData);
            
            // Process rewards and penalties
            const rewards = await this.processRaidRewards(battleResult, attackerData, targetData);
            
            // Set raid cooldown
            raidCooldowns.set(attacker.id, Date.now());
            
            // Create and send result embed
            const resultEmbed = await this.createRaidResultEmbed(battleResult, rewards, attacker, target);
            
            // Add rematch button for winner
            const components = battleResult.winner === attacker.id ? 
                [this.createRematchButton(attacker.id, target.id)] : [];
            ...
            if (components.length > 0) {
                this.setupRematchCollector(interaction, attacker.id, target.id);
            }
            ...
            const errorResponse = {
                embeds: [this.createErrorEmbed('An error occurred during the raid!')],
                ephemeral: true
            };
     ...
     */
    async validateRaid(attackerId, target) {
     ...
     */
    async getParticipantData(userId) {
     ...
     */
    async executeAutomatedBattle(attacker, target) {
        const battleLog = [];
        
        // Calculate battle stats
        const attackerPower = this.calculateBattlePower(attacker);
        const targetPower = this.calculateBattlePower(target);
        ...
        for (let round = 1; round <= RAID_CONFIG.BATTLE_ROUNDS; round++) {
            ...
            // Attacker's turn
            if (attackerHP > 0) {
                const damage = this.calculateDamage(attackerPower, targetPower);
            ...
            if (targetHP > 0) {
                const damage = this.calculateDamage(targetPower, attackerPower);
     ...
     */
    calculateBattlePower(participant) {
        ...
        const rarityBonus = this.calculateRarityBonus(participant.fruits);
     ...
     */
    async processRaidRewards(battleResult, attackerData, targetData) {
        const rewards = {
            berries: 0,
            fruitsStolen: [],
            experienceGained: 0
        ...
        if (battleResult.winner === attackerData.userId) {
            ...
            // Try to steal fruits
            const stolenFruits = await this.stealRandomFruits(targetData, attackerData.userId);
     ...
     */
    async stealRandomFruits(targetData, attackerId) {
        const stolenFruits = [];
        ...
        for (let attempt = 0; attempt < RAID_CONFIG.MAX_FRUIT_DROPS && availableFruits.length > 0 && stolenFruits.length < RAID_CONFIG.MAX_FRUIT_DROPS; attempt++) {
            ...
            if (Math.random() < dropChance) {
                try {
                    ...
                    await this.transferFruit(randomFruit, targetData.userId, attackerId);
     ...
     */
    async transferFruit(fruit, fromUserId, toUserId) {
     ...
     */
    async createRaidResultEmbed(battleResult, rewards, attacker, target) {
