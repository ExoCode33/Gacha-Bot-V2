...
/**
 * Helper function to get rarity basic attack bonus for display
 */
function getRarityBasicAttackBonus(fruit) {
    const bonuses = {
        'common': 1.0, 'uncommon': 1.1, 'rare': 1.2, 'epic': 1.3,
        'legendary': 1.4, 'mythical': 1.5, 'divine': 1.6
    };
    return bonuses[fruit.rarity] || 1.0;
}
    ...
    if (!selectedSkill) {
        ...
        raidState.attacker.team.forEach((fruit, index) => {
            if (fruit.currentHP > 0 && !isDisabled(fruit)) {
                ...
                const rarityBasicBonus = getRarityBasicAttackBonus(fruit);
